...
function detectPromiseResponse(content: string): boolean {
  ...
  const patterns = [
    ...
    /我\s*将?\s*(?:立即|马上|稍后|接下来|现在)\s*(?:亲自)?\s*(?:操作|修改|开始|添加|继续|进行|着手|尝试|执行|修复|编译|运行|处理|解决|完成|排查|调查|检查|调试|重试|测试|部署|启动|重启|删除|更新|创建|构建|打包|发布|安装|配置)\b/i,  // 我将立即亲自操作 / 我将尝试 / 我将执行修复
    /我\s*将?\s*(?:尝试|执行|修复|编译|运行|处理|解决|排查|调查|检查|调试|重试|测试|部署|启动|重启|删除|更新|创建|构建|打包|发布|安装|配置)/i,  // 我尝试 / 我将尝试 / 我将执行 (允许无中间"立即/马上")
    /我\s*(?:必须|一定|一定要|不得不|应当|需要|将要|打算|会[再]?)\s*(?:尝试|执行|修复|编译|运行|处理|解决|排查|调查|检查|调试|重试|测试|部署|启动|重启|删除|更新|创建|构建|打包|发布|安装|配置)/i,  // 我必须解决 / 我一定会修复
    /我\s*将?\s*(?:重新|再次|马上|立刻|立即|稍后|之后|最后)\s*(?:一次|回|遍|趟|次|步|个)?\s*(?:尝试|执行|修复|编译|运行|处理|解决|排查|调查|检查|调试|重试|测试|部署|启动|重启|删除|更新|创建|构建|打包|发布|安装|配置)/i,  // 我将最后一次尝试 / 我将重新尝试
    /(?:接下来|下面|然后|现在)\s*[，,]?\s*我\s*(?:会|将|会开始|将开始)\s*\w/i,  // 接下来，我会 / 接下来我将 / 现在，我将
  ];
